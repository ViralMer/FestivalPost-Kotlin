...
class BusinessDialogItemAdapter(var context: Context, var originaldata: ArrayList<CurrentBusinessItem?>) : RecyclerView.Adapter<BusinessDialogItemAdapter.BusinessItemAdapterViewHolder>() {

    var onItemClickListener: OnItemClickListener = context as OnItemClickListener

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BusinessDialogItemAdapter.BusinessItemAdapterViewHolder {
        val view: View? =
            LayoutInflater.from(parent.context).inflate(R.layout.custom_business_dialog_item, null,false)
        return BusinessItemAdapterViewHolder(view!!)
    }

    override fun onBindViewHolder(holder: BusinessDialogItemAdapter.BusinessItemAdapterViewHolder, position: Int) {
