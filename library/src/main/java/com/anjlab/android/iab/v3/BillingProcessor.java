...
public class BillingProcessor extends BillingBase
{
	...
	private SkuDetails getSkuDetails(String productId, String purchaseType)
	{
		...
		{
			Log.d("dddsdasdsa",""+skuDetailsList.get(0).priceValue);
	...
	private boolean verifyPurchaseSignature(String purchaseData, String dataSignature)
	{
		try
		{
			...
			return TextUtils.isEmpty(signatureBase64) ||
				   Security.verifyPurchase(signatureBase64, purchaseData, dataSignature);
		}
		catch (Exception e)
		{
			return false;
	...
	{
		return verifyPurchaseSignature(transactionDetails.purchaseInfo.responseData,transactionDetails.purchaseInfo.signature) && checkMerchant(transactionDetails);
	}
